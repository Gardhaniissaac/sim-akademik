...
<section class="content">
  ...
  <div class="card card-primary"> 
              <!-- /.card-header -->
              <!-- form start -->
              <form role="form" action="{{route('kelas.update',['kela' => $data->id])}}" method="POST">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="mata_kuliah">Nama Mata Kuliah</label>
                    <input id="mata_kuliah" name ="mata_kuliah" value="{{old('mata_kuliah', $data->mata_kuliah)}}">
                    @error('mata_kuliah')
                  ...
                  <div class="form-group">
                    ...
                    <input id="sks" name ="sks" value="{{old('sks', $data->sks)}}">
                    @error('sks')
                      <div class="alert alert-danger">{{$message}}</div>
                    @enderror
                  </div>
